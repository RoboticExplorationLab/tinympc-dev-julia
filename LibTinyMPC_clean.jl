...
function backward_pass!(solver::TinySolver)
    ...
    for k = (NHORIZON-1):-1:1
        Quu_inv = (work.R + work.Bdyn' * work.P[:, :, k+1] * work.Bdyn) \ I
        work.K[:, :, k] = Quu_inv * (work.Bdyn' * work.P[:, :, k+1] * work.Adyn)
        work.d[:, k] = Quu_inv * (work.Bdyn' * work.p[:, k+1] + work.r[:, k] + work.Bdyn' * work.P[:, :, k+1] * work.fdyn)
        work.P[:, :, k] = work.Q + work.K[:, :, k]' * work.R * work.K[:, :, k] + (work.Adyn - work.Bdyn * work.K[:, :, k])' * work.P[:, :, k+1] * (work.Adyn - work.Bdyn * work.K[:, :, k])
        work.p[:, k] = work.q[:, k] + (work.Adyn - work.Bdyn * work.K[:, :, k])' * work.p[:, k+1] - work.K[:, :, k]' * work.r[:, k] + (work.Adyn - work.Bdyn * work.K[:, :, k])' * work.P[:, :, k+1] * work.fdyn
    end
end
...
function update_linear_cost!(solver::TinySolver)
    ...
    work.p[:, NHORIZON] = -work.P[:, :, NHORIZON] * work.Xref[:, NHORIZON]
...
function reset_solver!(solver)
    ...
    solver.workspace.K = zeros(NINPUTS, NSTATES, NHORIZON - 1)
    solver.workspace.P = zeros(NSTATES, NSTATES, NHORIZON)
