...
function backward_pass!(params,X,U,P,p,d,K,reg,μ,μx,ρ,λ,λc,adaptive_step)
  ...
  for k = (N-1):(-1):1
      # dynamics jacobians (linear)
      if (adaptive_step > 0 && k > adaptive_step)
        A = ForwardDiff.jacobian(_x -> discrete_dynamics_stretch(params,_x,U[k],k),X[k])
        B = ForwardDiff.jacobian(_u -> discrete_dynamics_stretch(params,X[k],_u,k),U[k])
        f = discrete_dynamics_stretch(params,zeros(params.nx),zeros(params.nu),k)
      else
...
function forward_pass!(params,X,U,K,d,ΔJ,Xn,Un,μ,μx,ρ,λ,λc,adaptive_step; max_linesearch_iters=20)
  ...
  for k = 1:(N-1)
      Un[k] = - d[k] - K[k]*Xn[k] 
      if (adaptive_step > 0 && k > adaptive_step)
        Xn[k+1] = discrete_dynamics_stretch(params,Xn[k],Un[k],k)
      else
...
function mpc!(params,X,U,P,p,K,d,Xn,Un;atol=1e-3,max_iters=250,max_inner_iters=10,verbose=true,ρ=1,ρ_max=1e8,ϕ=10,adaptive_step=0)
  ...
  N = params.N
  for i = 1:N-1
    if (adaptive_step > 0 && i > adaptive_step)
      X[i+1] = discrete_dynamics_stretch(params,X[i],U[i],i)
    else
  ...
  ΔJ = backward_pass!(params,X,U,P,p,d,K,reg,μ,μx,0.0,λ,λc,adaptive_step)
  J, ΔJ = forward_pass!(params,X,U,K,d,ΔJ,Xn,Un,μ,μx,0.0,λ,λc,adaptive_step)
  ...
  for iter = 1:max_iters      
      ...
      for i = 1:max_inner_iters
          ΔJ = backward_pass!(params,X,U,P,p,d,K,reg,μ,μx,ρ,λ,λc,adaptive_step)
          J, ΔJ = forward_pass!(params,X,U,K,d,ΔJ,Xn,Un,μ,μx,ρ,λ,λc,adaptive_step)
          if verbose 
            @show ΔJ
          end
          if 0.0 <= abs(ΔJ) <= 1e-4 
